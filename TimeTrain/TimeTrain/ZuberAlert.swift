...
extension ZuberAlert{
    ...
    func showAlert(_ title: String, subTitle: String?,buttonTitle: String ,otherButtonTitle:String?,action:@escaping ((_ OtherButton: UIButton) -> Void)) {
        ...
        if buttonTitle.isEmpty == false {
            ...
            button.addTarget(self, action: #selector(ZuberAlert.doCancel(_:)), for: UIControlEvents.touchUpInside)
            button.tag = 0
        ...
        if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
            ...
            button.backgroundColor = MAIN_COLOR
            button.addTarget(self, action: #selector(ZuberAlert.pressed(_:)), for: UIControlEvents.touchUpInside)
